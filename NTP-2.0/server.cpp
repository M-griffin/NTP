...
bool NTPSRV::recvfinfo() {
    ...
    for (int i = 0; ;i++) {
		...
		if (tmpin[i] == '\n') {
        	...
        	if (pamcnt == 1) {
                strcpy(finfo[0][NODE].bbsver,tmppak);   memset(tmppak,0,sizeof(tmppak)); ab = 0;
       		...
       		// Check for Filename String
				...
				// Get True Filename and cute off Path if found!!
  				...
  				if (num == 0) { strcpy(finfo[0][NODE].filename,arg); }
   				else {
        			int r = 0;
           	...
           	// Check for Filesize String
				...
				strcpy(arg,tmppak);
         		finfo[0][NODE].size = atol(arg); memset(tmppak,0,sizeof(tmppak)); ab = 0;
				sprintf(message,"Filesize: %i",finfo[0][NODE].size);
				logntp(message, NODE);
            }
            // Check for Batch Queue String
        	else if (pamcnt == 4) {      	    
         		strcpy(finfo[0][NODE].bqueue,tmppak); memset(tmppak,0,sizeof(tmppak)); ab = 0;
...
void NTPSRV::recvfdata() {
	...
	while (j>0)	{
		...
		if (nRet == INVALID_SOCKET)	{
			...
			for (int rtry = 0; ; rtry++) {
				...
				if (rtry == 10) {
                    ...
                    if (errno != WSAEWOULDBLOCK) {
                    #endif
					    if (finfo[0][NODE].size == totbyte) { j = 0; break; }
		...
		if (nRet == SOCKET_ERROR)	{
            ...
            if (errno != WSAEWOULDBLOCK) {
            #endif
			    if (finfo[0][NODE].size == totbyte) { j = 0; break; } // Exit Casue File is Finished
			    sprintf(message,"Disconnected!");
			    logntp(message, NODE);
			    strcpy(finfo[0][NODE].status,message);
			    memcpy(&finfo2[0][NODE],&finfo[0][NODE],sizeof(FILEINFO));
...
void *DLNodeTransfer( void *p ) {
	...
	else if(i == 0) { finfo[0][ntpsrv->NODE].resum = false; }	
    else { finfo[0][ntpsrv->NODE].resum = true; }
...
void StartServer(short nPort) {
    ...
    */
    ...
    while(1) { StreamServer(nPort, listenSocket); }
}
